...
### 外部 review 建議的處理原則（2026-07 知識庫 v3 review 定案）

處理任何外部 UX/CRO 稽核報告時，固定兩步：**「問題診斷」逐項對照現況檢驗、「解法處方」不照抄**——先對照品牌 token、既有元件與程式碼證據再決定採不採。先例與完整對照表見 `docs/WEB_REVIEW_V3_GRAD_CHECKLIST.md`（該輪 30 項建議中 10 項現況早已符合、多數處方需改寫成符合站上系統的版本）。

以下三條是該輪定案的**固定紅線**，日後 review 再提同類建議時直接引用本節理由，不重新辯論：

1. **不照抄 review 指定的色碼**（#F4F7F9、#737373、「深藍」之類）。
   一律映射既有 `--tbd-*` token，且採用前先驗對比度——v3 review 建議把灰字加深到 #737373，但站上 `--tbd-mid` 早已是 #6A6D89（白底 4.9:1，過 WCAG AA），照抄反而**變淺**。寫死色碼也會繞過 token 系統，日後改品牌色必漏。
2. **guide-rail 大數字不改絕對定位**。
   review 診斷「大數字佔排版流導致標題錯位」的前提不成立：rail 用 grid 固定欄寬（`--railnum`），所有標題本就對齊同一垂直基準線；數字色 `--tbd-light` 已是低視覺權重。絕對定位＋負 margin 只會增加窄螢幕溢出風險，目的已達成就不動。
3. **不做 8pt 網格全域重排**。
   「全站段落與間距對齊 8 的倍數」屬全站排版系統重構，違反單輪 Scope 紀律與「不任意重構」紅線。間距問題以**局部對齊**處理（例：文章章節間距調成 48px），全域重排留待真正的大改版另案評估。

> 反向先例：不採 ≠ 永久否決。列表圓點改自訂標記原判不採（單一系列改會造成不一致），使用者核准「全站統一」後即執行——但實作仍換成符合站上慣例的形式（純 CSS 品牌色小菱形，不引外部 SVG）。原則是：**採納與否聽使用者，實作方式守站上系統**。
