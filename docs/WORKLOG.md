...
## #051｜2026-09-24｜新增「這些工具為什麼這樣設計」頁（9/23 會議決議 I）

**問題**：會議決議把工具的理論依據整理到官網，重點是說明工具「為什麼這樣設計」，而不是列理論名稱。來源是團隊的方法論筆記（四個理論收斂成三根支柱），但那份筆記寫給顧問用，含顧問訓練、48 小時承諾、內部稽核與對外話術，不能直接上站。

**改了什麼（可觀察）**：
- 新頁 `/pages/resources/tool-design.html`：三個原則（方向是說出來的／幫忙要有退場的設計／把焦慮變成看得見的下一步），每個原則寫理論一句話、在工具裡長成什麼樣子（點名具體工具與欄位）、刻意不做的事；最後附 9 筆出處
- 出處 7 筆附 DOI，逐筆對 Crossref 核對標題、期刊、年份、卷期、頁碼，doi.org 皆回 302；2 本書不附連結
- 方法論筆記把「相當一部分回饋介入反而讓表現變差」歸給 Hattie，實際出處是 Kluger & DeNisi (1996)，頁面上改掛正確出處
- 下載頁與 9 個分學群指南頁的工具包說明句尾加「這些工具為什麼這樣設計 →」
- `verify.mjs` 加這頁的煙霧測試

**Non-scope**：顧問服務本身的方法論（12 週路線圖、模擬面試回饋報告等）不放這頁；頁面只講工具。

**驗證**：build 通過；verify 41/41；本機 preview 截圖 1280 與 500 寬，卡片與清單無溢出。

**風險與待確認**：頁面舉例用到 v1 活頁簿才有的機制（淡黃格、我到哪了、雷達圖），所以跟 #40 一起上線，不單獨發。內容待 YY／CL 過目（會議指定負責人）。

**下一步**：與 #40 一起合併；上線後抽查這頁與三個入口連結。

## #050｜2026-09-24｜56 份模板全部改發 Excel 活頁簿 v1，撤掉 .md／.csv（D-021）

**問題**：9/12 起審閱改以活頁簿交件，站上 53 份仍發 .md／.csv 舊版；9/24 LR 拍板的規格統一只做在 xlsx 上。

**改了什麼（可觀察）**：
- `public/assets/templates`：換上 56 份 xlsx（Drive「03_輪調後格式統一版本_修正v5」，五輪修正後的版本），刪掉 106 個 .md／.csv
- `template-manifest.json`、`gradTemplates.ts`：56／45 筆全部 `xlsx: true` ＋ `xlsxOnly: true`
- 下載頁、9 個分學群指南頁：按鈕只剩一顆「下載 Excel 活頁簿」；「格式任選 CSV／Markdown」說明改成活頁簿用法；「怎麼變成你自己的正式模板」改寫成 Excel／Google 試算表／Notion 嵌入三種用法；高中 10 份與甘特圖的站上名稱對齊檔案標題
- 文章 29 篇 36 處下載連結改指 .xlsx，連結文字改成現行工具名稱，「（Markdown，可匯入 Notion）或 CSV 版」這類並列格式收成單一連結
- compass、知識庫首頁的「CSV 與 Markdown」描述改掉
- `audit-templates.mjs`：「6 欄」下限只驗 CSV；新增活頁簿作者欄位掃描（上架前 50 份帶成員姓名，已清成 TBD Studio；植入測試名會讓 --strict exit 1）
- `verify.mjs`：4 個煙霧測試的 mustContain 從 .md 改成 .xlsx，「套磁」測試名改「聯繫信」
- CLAUDE.md「新增分學群模板」步驟改成只放 xlsx

**Non-scope**：新增的草稿內容還在等 CL／YY 審（教育備審 4 項、生醫／商管／法政計畫書、口試 5 則區尾說明）；審完只需換檔，不必改程式。

**驗證**：build 176 頁；verify 40/40；dist 內無成員姓名、無 .md／.csv 模板連結；本機 preview 截圖確認下載頁按鈕。

**風險與待確認**：Notion 使用者失去一鍵匯入；若反映多再評估由 xlsx 產生 .md。本分支疊在 #39 上，合併順序 #38 → #39 → 本 PR。

**下一步**：CL／YY 審完後，把修正檔換進 `public/assets/templates`（作者欄位閘門會擋未清的檔），再合併本 PR；上線後抽查三份下載與一篇文章連結。

## #049｜2026-09-24｜研究所工具名與 xlsx 標題對齊：「學群｜共同名：特色」

**問題**：45 份研究所工具中有 21 份網站名稱與檔案標題不同（例：網站「研究計畫可行性檢核表」、檔案「研究計畫書架構模板」），使用者下載後對不上；銷售頁若成組列出也會像不同產品。

**決議（LR，9/24）**：保留各學群特色，標題統一為「學群｜共同工具名：學群特色」，網站與檔案一字不差。共同名：校所與教授比較表／聯繫信結構模板／研究計畫書架構模板／備審檢核表／口試自我檢核表；只有真的做法不同的才加「：特色」。

**改了什麼（可觀察）**：
- `gradTemplates.ts` 45 個 title 全部換成與 xlsx A1 相同的字串（xlsx 另加「　v1」，網站不加）
- 文章內 13 處下載連結文字改成新名稱（文章已屬單一學群，省略學群前綴）；兩篇 description 的「附可下載 Checklist」改「檢核表」

**Non-scope**：高中工具（沒有學群）；文章裡連到 .md／.csv 的連結本身（等 xlsx 上架一起換）。

**驗證**：verify 40/40。

**風險與待確認**：本分支疊在 #048（套磁→聯繫）之上，#048 先合併。

**下一步**：xlsx 上架時，檔案 A1 與這裡的 title 用同一份對照（scratchpad 的 titles.json 已產出，上架時轉成正式檢查）。

## #048｜2026-09-24｜全站「套磁」改「聯繫」（9/23 會議決議 J）

**問題**：9/23 會議決議「套磁」不符目前品牌語境，全站統一改為「聯繫」。CL 9/23 交付的 56 份 xlsx 已先改完，頁尾搭配文章標題寫成「研究所聯繫教授：聯繫信怎麼寫、什麼時候寄」，站上文章標題還是舊詞，兩邊對不上。

**改了什麼（可觀察）**：
- 44 檔、125 處：文章 22 篇、頁面 4 個、`gradTemplates.ts` 工具名 3 個、`llms.txt`、`public/assets/templates` 的聯繫信 md／csv 17 份
- 直接替換會不通順的 8 處改寫：刪掉「（俗稱「套磁」）」；「聯繫教授（套磁）信」→「聯繫教授：聯繫信」；「用套磁信聯繫教授」→「寫聯繫信給教授」；「想聯繫教授，卻怕一封套磁信石沉大海」→「想寫信給教授，卻怕石沉大海」；另 4 處避開同句重複「聯繫」
- 其餘逐行讀過，直接替換語意不變

**Non-scope**：`docs/` 內部規劃與歸檔文件的舊詞（不上站，約 30 處）；URL slug 本來就是英文，不動。

**驗證**：verify 40/40；`src`、`public` 內「套磁」0 處。

**風險與待確認**：「套磁信」是搜尋關鍵字，改名後 SEO 可能有短期影響；要不要在 contact-professor 文內保留一次「又稱套磁」由團隊決定（本 PR 已拿掉）。

**下一步**：PR 合併後打正式站 graduate-contact-professor 確認標題；56 份 xlsx 上架時不需再改這個詞。
...
**補（同日）**：LR 確認免費諮詢 15 分鐘，已寫進對照表、FAQ（頁面＋JSON-LD）、首頁 hero 說明與價格卡文字連結；按鈕不加時長。PR #36 合併（`8c2ee16`），正式站首頁 3 處、FAQ 3 處、服務頁 1 處可見。

**真機驗收**：LR 手機目視首頁價格卡與對照表，無跑版。

**下一步**：本輪結案。GA 比較免費／付費轉換時切在 2026-09-16。
...
那條規則假設「一天一版」，但 2026-09-12 是邊修邊掃——每修完一批就重跑一次確認狀態，
四小時內跑了十一次，`progress/` 於是長出 v3–v13 十一份內容高度重疊的檔案，
沒人知道該看哪一份。

**關鍵判斷**：報告是**狀態快照不是增量**，後一版本來就涵蓋前一版，
所以那十一份裡有十份是已被取代的中間態。合併不會遺失狀態。

**但合併不等於只留最後一份**：那十份唯一不重複的資訊是**推進過程**
（哪一批修完、修了幾項）。新的 v3 內容取自最後一次掃描（原 v13，20／0／2），
另外新增「00 版本合併說明」一節，用表格記下十一次各自的時間與已修／未修／待拍板。
**表格數值由各檔案程式化抽出，沒有一個是手打的**——理由和產生器本身的設計相同：
手寫的數字修完不會有人回頭改，報告就開始說謊。

**可逆**：原始十一份移到 `progress/_archive/`，沒有刪除。

**驗證**：新 v3 標題／版號標示一致、headline 為「已修 20、未修 0、待拍板 2」、
推進表 11 列、`<section>` 與 `<table>` 標籤配對、節序 00→06 連續；
現存版號 1,2,3，產生器下次續號 v4（取最大值 +1，不會撞號）。

**計畫／決策異動**：新增 **D-019**（同日連續重掃合併成一版；跨日才開新版號）。
同一段說明寫進 `build-seo-progress.mjs` 檔頭，讓照原規則跑的人看得到例外。

**風險與待確認**：`progress/` 不在任何 repo 內（見 tbd-root-docs-ungoverned），
所以 D-019 有版控、它約束的腳本沒有。腳本檔頭那段註解是目前唯一的在地提示。

**下一步**：S-09 正式網域（BIZ-01）與 A-02 具名作者兩項待拍板仍卡在決策，不是技術。
下次跑產生器會是 v4；若又出現同日多次重掃，依 D-019 在當天結束時合併，不要累積。

---

## #044｜2026-09-12｜A-04 完成，SEO 22 項未修歸零（20 已修／0 未修／2 待拍板）

**A-04 我原本擋著不做，理由是「表格是新增資訊」。** 那個判斷只對了一半——
正確的做法不是編表格，是**把文章裡本來就是表格形狀、卻寫成條列的段落改成表格**。
那是重構既有內容，而 A-04 要的「可抽取的結構」正是這個。

**做法（關鍵是零轉寫）**：寫了一支 `to-table` 腳本，**表格內容完全由既有條列生成**，
我只提供兩件事——要轉哪個段落、兩個欄位標題。所以整個過程沒有經過我的重新打字，
不可能改到原文。它同時拒絕已有表格的檔案、條列不足三項的段落。

挑選判準：只轉「一個維度 × 多個項目」而且格子夠短的段落。長文字塞進表格在手機上更難讀，
所以不是每個條列都該變表格。實際轉了 38 篇、含表格從 21 → **62／134**。

最漂亮的一個是 `biomed-graduate-cv` 的「技術清單怎麼寫」——原文本來就是
「避免只列／建議具體化」交替的對照，腳本用 `mode: pairs` 兩兩併成一列，直接變成一張對照表。

**順手抓到一個我自己造成的假通過**：轉完後 S-11 退回未修（`ai-era` 950、`github` 982、
`interview` 960 字），每頁剛好掉約 60 字——那正是 #043 修掉的全站漏字長度。
**S-11 上一次通過，有一部分是被那段 bug 文字撐上去的。**
漏字修掉後真實字數才露出來。三頁各補一節真正的導讀（怎麼讀這個系列、評審實際看什麼、
三種面試的差別），不是灌字數。

**驗證**：build 176 頁通過；verify **40/40**（含 D-009 的表格 RWD 回歸與 400px 溢出檢查）；
dist 抽查五篇新表格都渲染成 `<table>`、無殘留字面 `|`；
SEO 掃描 **19／1／2 → 20／0／2**（`v13_20260912`）。

**剩下兩項都不是我能決定的**：

- **S-09 正式網域**（稽核 BIZ-01）——影響 UTM 與 GSC，要先有網域決策。
- **A-02 具名作者**——Article schema 的 author 目前一律是 Organization。升學建議屬 YMYL 邊緣領域，
  掛具名作者要有真實可查證的資歷，那是品牌與人事決定，不是技術問題。

---

## #043｜2026-09-12｜修兩個上線後才被看見的錯：全站漏字、重點框寬度

兩個都是我造成的，使用者在正式站上看到的。

**一、全站每一頁都漏出一段註解文字（嚴重）**

`BaseLayout` 那段 Tailwind 註解裡，我把 `{/* */}` 與 `<!-- -->` 寫進了註解**內文**。
**JSX 註解不能巢狀**——內文裡的結束符號提前關掉註解，後面整段就變成頁面上看得見的文字：

> 而不是 HTML 註解：Astro 的 會原樣輸出，而註解裡若出現 CDN 的網域字串，SEO 掃描器會誤判成這頁還在載 CDN。 */}

而且它在 `BaseLayout` 的 `<head>`，所以**172 頁全中**。正式站三頁抽查都有。

諷刺的是這段註解本身就是在解釋「HTML 註解會外流」——我為了避免上一個坑，用了會產生下一個坑的寫法。
改寫成不含任何結束符號的版本，並在註解裡寫明這條規則。
另外寫了一支檢查掃過全部 `.astro`：其餘 JSX 註解都沒有同樣問題。

**為什麼閘門沒擋住**：`verify` 驗的是「每頁零 JS 錯誤 ＋ 互動可用 ＋ 無水平溢出」，
漏出來的純文字三項都不違反。SEO 掃描器也只找特定字串。**這種錯只有人眼看得到。**

**二、重點框寬度與內文不一致**

原本把它放在 `<main>` 外面，沒吃到 `.sub-page main` 的容器寬度，撐成整個視窗寬。
第一次修把它移進 `<main>`，但那是兩欄容器（1076px），仍比內文欄（764px）寬一截。
最後放進 `.article-body` 內、`<slot />` 之前。

實測（CDP 量 `getBoundingClientRect`，不靠目視）：
...
**驗證**：build 176 頁；verify 40/40；dist 漏字字串 0 次；重點框仍正常渲染。

**留給下次的兩條**：

- JSX 註解不能巢狀，**內文不要出現註解結束符號**，否則會提前關閉並把後面變成可見文字。
- 我的一次性 CDP 探針用 `spawn(..., {shell:true})` 起 `astro preview`，
  Windows 上 `kill()` 只殺得到 shell，會殘留占著 4321 讓下一次 verify 假失敗。
  用 `netstat -ano | findstr :4321` 找（它綁 IPv6，`Get-NetTCPConnection` 抓不到），或改用 detached＋PID 殺。
