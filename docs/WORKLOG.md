...
## #042｜2026-09-12｜A-06 完成：134／134 篇都有答案先行的重點框

接續 #040 的機制，把剩下的 94 篇寫完。全站 134 篇現在都有「這篇的重點」框。

**內容怎麼來的**：不是我憑印象寫。每批先用腳本抽出該篇的 `lead` 與**每個 section 的第一段**，
再據此改寫成三到五條。抽取腳本一開始被 CRLF 卡住（section 內文抽不到、只抽到 lead），
修掉 `\r?\n` 之後才拿到真正的依據。

**寫作判準**：每一條都要是文章裡講過的具體主張，不是標題的改寫。
所以會出現「個案題最常見的失分回答是『我會很有耐心地陪伴他』——這句話不錯，但沒有任何專業內容」
這種句子，而不是「說明個案題的答題方式」。**不新增文章沒講的主張、數字或承諾。**

**驗證**：build 176 頁通過；verify 40/40；
dist 抽查 `why-busy-not-strong` 與 `three-year-plan` 都有 `article-takeaways` 區塊；
SEO 掃描 **已修 18／未修 2 → 已修 19／未修 1**（`v11_20260912`）。

**SEO 22 項目前的狀態**：

| 狀態 | 項目 |
|---|---|
| ✓ 已修 19 | S-01–S-08、S-10–S-14、A-01、A-03、A-05、A-06、A-07、A-08 |
| ✗ 未修 1 | **A-04 可抽取的表格結構偏少**（21／134，目標 ≥ 60） |
| · 待拍板 2 | S-09 正式網域、A-02 具名作者 |

**A-04 為什麼留到最後**：它要求 39 篇文章新增真實表格。這不是格式工作——
表格裡的每一格都是內容，要寫得正確就得對該篇主題有實質判斷。
其餘 21 項我都能從既有內容推導出來（重點框抽 section、HowTo 抽步驟、描述回復 git 原文），
但表格不行，它是**新增資訊**。這一項我認為該由 YY／LR 決定要不要做、以及由誰寫。

---

## #041｜2026-09-12｜修正 #039 的偏差：88 篇 description 補回視覺寬度

**這則在修我自己前一輪做過頭的事。**

#039 把 92 段 description 收到 90 字以下時，我照的是 SEO 掃描器的判準（`desc.length > 90`，
**數碼點**）。但這個專案自己有一份 skill——`.claude/skills/tbd-seo-meta`——它的判準是
**視覺寬度**（中文算 2、英數算 1），目標區間 **150–180 寬**，並明說偏短會浪費版位、CTR 偏低。

跑了 skill 自己的 `check_meta.py` 才發現：0 個 ❌（沒有截斷風險），但
**88 篇 ⚠️「description 偏短」**，寬度 107–138、中位 130，全部低於下限。

**兩個標準其實可以同時滿足**：純中文 90 字 ＝ 180 寬，正好是甜蜜點上緣。
所以目標訂成「寬 150–180 且字數 ≤ 90」，我先前把目標壓在 62–67 字（124–134 寬），是低估了。

**怎麼修的**：不是重寫，是**從 git 把我改短前的原文取回來，再修到字數 ≤ 90**。
原文多半本來就落在 150–180 寬，只是字數超過 90——那才是它們當初被 A-08 抓到的原因。
腳本每批套用後自動回報「新描述本身是否仍超過 90 字」，超過的當場修掉（共修了 7 次）。

**一個兩個標準真的會打架的案例**：`github-commit-guide` 的描述 ASCII 很多
（`GitHub`、三個 `commit`、`update`），字數高但寬度低——92 字卻只有 148 寬，
兩邊同時卡住。減少一次 `commit` 重複就同時過關。**這類文章只有在 ASCII 佔比高時才會撞到。**

**結果**：

| 指標 | 前 | 後 |
|---|---|---|
| `check_meta.py`（skill 的寬度判準） | 88 篇 ⚠️ 偏短 | **134 篇乾淨，0 ❌ 0 ⚠️** |
| A-08（掃描器的字數判準） | 0 頁超標 | **0 頁超標**（維持） |

**驗證**：build 176 頁通過；verify 40/40；SEO 掃描維持 18／2／2（`v10_20260912`）。

**留給下一個人的提醒**：`progress/build-seo-progress.mjs` 的 A-07／A-08 用字數判定，
`tbd-seo-meta` 用視覺寬度。**兩者對純中文接近等價，對 ASCII 多的標題／描述會分歧。**
要動這兩項時以 skill 為準（它描述的是真實的 SERP 行為），掃描器當成下限守門。
掃描器要不要改成量寬度，留給使用者決定——我沒有為了讓自己的工作過關而去動量測工具。
...
一開始以為要改 129 頁，量過之後發現不是：178 頁裡有 **152 頁把「｜TBD Studio」手寫進 title**，
後綴佔 11 字。**把後綴拿掉後，本身仍超過 30 字的只有 6 頁。**

所以改法是結構性的，不是逐頁改：

- **`BaseLayout` 接管品牌後綴**：`baseTitle.length + 11 <= 30` 才接，接不下就讓標題把 30 字全用在內容上。
  結尾另加 `.replace(/｜TBD Studio$/, '')` 防呆，來源檔若又被手動接上後綴也不會接兩次。
  品牌訊號不會因此消失——`og:site_name` 與 Organization schema 都還在。
- **來源檔一律不帶後綴**：140 篇 `.mdx` 的 `seoTitle` ＋ 12 個 `.astro`。
  其中 7 篇的值用引號包住，第一支 regex 沒吃到，補了第二支。
- **手改 6 個本身就超標的標題**（36/33/32/31/31/31 → 29/27/25/24/26/26）。

**過程中弄壞又修回的一件事**：`side-project-types.mdx` 的 `title` 與 `seoTitle` 原本字串相同，
我的取代命中了第一個，也就是 **H1**。已把 `title` 還原、改動 `seoTitle`，
`git diff` 現在只剩 seoTitle 一行。**H1 是可見文案，不在這輪範圍內。**

**A-08 meta description 超過 90 字：92 頁 → 0**

這批沒有結構性解法，92 段是逐篇重寫的——保留「這篇回答什麼 ＋ 交付什麼」，砍掉鋪陳。
**沒有用機械截斷**，截斷會產生斷句。最長的一段從 133 字收到 69 字。
套用時按**檔案**定位而不是比對舊字串，避免改錯行；腳本會回報新描述本身是否仍超標（0 筆）。

**驗證**：build 176 頁通過；verify **40/40**；
SEO 掃描 **已修 11／未修 9 → 已修 13／未修 7**（`v6_20260912`）。

**一個自己造成的小汙染**：產生器跑了兩次，多出一份內容相同的 `v7`。
確認除版號外完全一致後移除，series 維持到 v6。

**剩下 7 項**：S-03（Tailwind CDN，架構級）、A-04 表格、A-05 HowTo、A-06 重點框、
S-11 薄頁、S-12 內部連結、S-14 建置期抓取。待拍板仍是 S-09 網域與 A-02 具名作者。

**下一步**：A-05 只需要有一篇掛上 HowTo 就會翻綠，但要做得誠實應該挑真正是步驟型的文章
（研究所 7 階段、30 天檢核、各 timeline）補 `howToSteps`，不是為了過閘門硬掛一篇。
...
釐清後確認**兩者不是同一個軸、也不互相取代**：面試三件套是同一個服務的強度分級，
方案 A–D 是跨服務類型（備審健檢／備審重構／面試訓練／全程陪跑）。
所以不是產品衝突，是同頁出現兩組 A/B/C 會讓讀者誤以為有對應關係。
LR 決定不等 YY 回覆，直接拿掉字母。

**做了什麼**：

- `figures.ts`：`interviewPlans.plans[]` 移除 `label` 欄位（原值 `PLAN A/B/C`），
  型別同步收掉。三個名稱「基礎診斷方案／實戰衝刺方案／尊榮全套陪伴方案」與「熱門推薦」
  徽章都保留，**其餘文案一字未改**。
- `PlanCompare.astro`：不再渲染 `.plancmp-label`。
- `tbd-pages.css`：刪掉隨之失效的 `.plancmp-label` 規則（不留死 CSS）。

**順帶修掉一個我自己製造的問題**：#034 在 `services.astro`／`process.astro` 插入區塊時
用了 `<!-- -->` 註解。**Astro 的 HTML 註解會原樣輸出到 dist**，所以那三段內部備註
（含 Drive 文件名「SOP 圖檔放置」、YY 的名字、以及「兩套命名尚未收斂」這種內部討論）
本來會出現在正式站的原始碼裡。全部改成 `{/* */}`，這種不會輸出。
dist 實測：`PLAN`／`SOP 圖檔放置`／`YY` 在兩頁皆為 0 次。

**驗證**：build 176 頁通過；verify 40/40；
dist 抽查三個方案名稱與「熱門推薦」各 1 次、急件說明句 1 次，內容沒被誤刪。

**下一步**：#034 的兩件待決事項至此清空（急件見 D-018、字母見本則），可併入 main 部署。
YY 尚未對「拿掉字母」表態，若他要求改回，只需把 `label` 加回 figures.ts 與元件。

---

## #036｜2026-09-12｜急件定案為加成而非方案，服務頁補一句說明

LR 拍板：急件收方案費用 +20%，但**對外不標價**，照站上慣例寫「初談時說明」（D-018）。

**做了什麼**：`services.astro` 的 `#urgent` 區塊，在急件 CJM 之後加一個 `.note`：

> 急件不是另一套服務，而是同一套流程在更短的時間內執行，因此不會另立方案。是否能承接、
> 要壓縮哪些環節，以及費用怎麼計算，都會在初談時依剩餘時間與實際需要處理的範圍一併說明。

這句解掉 #034 留下的「站上沒有急件服務、圖卻在宣告一個查不到內容的方案」。
它同時說明了為什麼急件不出現在「怎麼開始合作」的方案 A–D 裡——急件是加成，不是第五個方案。

**沒有寫進頁面的數字**：+20% 與加成基準。LR 一度提「最短三個月」，但站上
`cases.astro` 公開掛著 `合作時長：約 2 週（急件）` 與「十四天急件修改排程」，
兩者直接對撞；改採「初談時說明」後這個矛盾不存在，也不必動案例頁。
要之後改成公開標價，得先確認三個月是「急件門檻」還是「可承接下限」，
並同步處理案例頁——見 D-018。

**驗證**：build 176 頁通過；verify 40/40。

**仍未決**：圖上的 `PLAN A/B/C` 與方案 A–D 撞字母。已確認兩者不衝突
（面試三件套與服務四方案是兩條並行階梯，不互相取代），所以縮小成版面問題：
擬拿掉圖上的字母標籤、只留「基礎診斷／實戰衝刺／尊榮全套」。**動的是 YY 的內容，等他點頭。**
...
實際的走法是：**審閱者從正式站的網站連結看當次的檔案**，把修正後的新版本放進
Drive 的「審閱中」資料夾，再由收斂者匯出 CSV 走 PR 回 repo。
差別在**誰產生 Drive 上那份檔案**——不是收斂者預先鋪好，是審閱者放上來的。

**做了什麼**（可觀察的行為差異）：

- **`docs/DECISIONS.md` D-015「選了什麼」改寫**。照舊條文做，收斂者會先從 repo 的 CSV
  產生一批工作副本鋪進「審閱中」；照新條文做，他只建空資料夾並發網站連結。
- **「被否決的」新增一條**：由收斂者預先鋪工作副本。理由是預鋪只會多出一份沒人動的
  中間版本，反而製造「哪一份才是他改的」這個問題。
- **`docs/review-workflow.md` 第一節的方向圖改成四段迴圈**
  （`CSV（repo，正本） → 正式站 → Drive「審閱中」 → 收斂者匯出 CSV 走 PR`），
  原本是兩段（`CSV → Sheet`，修正回到 CSV）。
- **「一輪審閱」的第 1–3 步重寫**。第 1 步從「產生工作副本丟進資料夾」改成
  「建空資料夾＋發該輪的網站連結」；第 2 步從「在自己那份 Sheet 上改」改成
  「從網站連結看檔，把修正後的新版本放進該輪資料夾」。第 4–8 步不變。
- **「兩個資料夾」表的『放什麼』與『何時建』跟著改**：審閱中放的是
  「審閱者放上來的修正版本」，建的是空資料夾。
- **「已知的坑」新增一條**：入口是正式站，而正式站部署的是 `main`——
  工作還在 feature branch 就等於審閱者看到的不是最新的。發連結前要先確認該輪的改動已部署。

**沒有動的**：repo 端檔名不分岔（`{slug}.csv`）、Drive 端命名規則、
凍結判準（D-013）、第 4–8 步的閘門順序。這次只改「檔案怎麼進到 Drive」那一段。

**憑什麼說做完了**：只動兩份 docs，不碰程式碼、模板或 manifest，閘門與 build 不受影響。
PR #34（`34cd1b4`）squash 進 main。

**注意**：#031 引用的是舊條文的說法（「Drive 上改、匯出 CSV 走 PR 回 repo」）。
歷史紀錄不追改，以本則為準。
