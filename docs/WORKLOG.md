...
## #030｜2026-09-05｜Batch 2 輪調審閱落地（一）：文章三處對齊工具，理工連結從表格資料列移到頁尾
...
- **理工的通用篇連結從表格資料列移到頁尾**（`grad-engineering-contact-email.csv`）。
  原本在第 24 列，位置落在「常見的石沉大海原因」表的資料列區，匯入 Google Sheets 後
  會變成那張表的一部分。九份裡只有理工這一份連回通用篇，其餘八份是否比照辦理，
  等優化版落地時一起決定，這次不擅自擴散。
...
| 閘門 | 結果 | 證據 |
|---|---|---|
| `audit-templates.mjs --strict` | PASS | 45 份規格全 ✓；理工 contact-email 行數 38、欄數 6，BOM 未動 |
...
1. **十五格共同規格缺口與四項待拍板的新增，全部未做。** YC 的優化版 CSV 9 份在她 Drive 的
