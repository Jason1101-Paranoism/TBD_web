...
## #052｜2026-09-25｜工具頁與資源頁的學群數：「六個學群」→「九個學群」，刪除「設計傳播尚未附模板」

**問題**：`gradTemplates.ts` 早已是九個學群各五份（設計傳播 2026-08-21 補齊，正式站可下載），但兩處對外文字停在舊狀態：
`tools.astro` 的頁面描述寫「六個學群各五份」、頁尾寫「設計傳播類群……尚未附模板」；`resources.astro` 的分類描述也寫「六個學群」。
商賽企劃書寫「九學群、46 份工具」，評審點進工具頁會看到矛盾。

**做了什麼**：三處改為九個學群（理工、生醫與公衛、商管財經、人文社科、藝術、教育、法政、設計傳播、農生環境），頁尾改為「找不到你的學群？可先用通用七階段對照表……」。

**憑什麼說做完了**：`npm run build` 176 頁完成；`dist/pages/resources/tools.html`、`dist/pages/resources.html` 各含「九個學群」1 次、「六個學群／尚未附模板」0 次；`npm run verify` 40/40 通過。
設計傳播五份模板檔在 `public/assets/templates/` 與正式站 `tools.html` 皆可下載（事先確認）。

**下一步**：合併部署後在正式站目視 `tools.html` 頁尾；`feat/xlsx-launch-v1`（#40）合併時注意這兩行不要被舊版覆蓋。
