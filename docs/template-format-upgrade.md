...
- ~~`scripts/verify.mjs` 的模板閘門 —— 目前 `templateFiles()` 掃的是磁碟上的 `.md`，
  ...
  **已解決（2026-08-09，D-008）**：事實來源改為 `scripts/template-manifest.json`，
  與磁碟雙向校驗。改形式時把該筆改成 `delivery: "external"` 並填 `url`，
  下載頁上必須出現那個 url——**只刪磁碟檔會直接讓閘門紅**，四個失敗方向都已實測。
...
5. ~~想清楚模板閘門在「模板不再是磁碟檔案」之後的事實來源（見第四節）~~
   **已完成（2026-08-09，D-008）**

> 本節與 `docs/PLAN.md` 的 Phase 3 是同一份待辦。PLAN.md 是跨主題的正本
> （開場簡報讀它），這裡保留形式補齊的細節與逐份對照表。
